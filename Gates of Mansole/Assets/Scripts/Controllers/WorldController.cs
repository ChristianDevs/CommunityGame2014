...
public class WorldController : MonoBehaviour {
	...
	void handleRowDamageAbility() {
		...
		if ((tile.row != -1) && (tile.col != -1) && (currentLevel.GetComponent<WaveList>().laneEnable[tile.row])) {
			newPos = new Vector3(map.transform.position.x,
			                     map.transform.position.y + ((float)tile.row * map.transform.localScale.y) + TileUnitOffset);
			selectedUiAbility.transform.position = newPos;

			if (placementBoxes != null) {
				foreach (GameObject pb in placementBoxes) {
					Destroy(pb);
				}
			}
			...
			placementBoxes = new List<GameObject>();
			for (int col = 0; col < gridSize.col; col++) {
				Vector3 boxLoc = map.GetComponent<UiTiles>().lanes[tile.row].GetComponent<UiRow>().rowTiles[col].transform.position;
		...
		if (Input.GetMouseButtonUp(0)) {
			...
			if ((tile.row >= 0) && (tile.row < gridSize.row) && (currentLevel.GetComponent<WaveList>().laneEnable[tile.row]) &&
			    (selectedAbility.GetComponent<Ability>().getUseCost() <= Player.spiritShards)) {
				float xTilePos;
				float yTilePos;
	...
	void handleRadiusDamageAbility() {
		...
		if ((tile.row != -1) && (tile.col != -1) && (currentLevel.GetComponent<WaveList>().laneEnable[tile.row])) {
			...
			for (int colInd = tile.col - selectedAbility.GetComponent<Ability>().getAreaOfEffect();
				...
				Vector3 boxLoc = map.GetComponent<UiTiles>().lanes[tile.row].GetComponent<UiRow>().rowTiles[colInd].transform.position;
			...
			for (int rowInd = 1; rowInd <= selectedAbility.GetComponent<Ability>().getAreaOfEffect(); rowInd++) {
				...
				for (int colInd = tile.col - rowRange; colInd < tile.col + rowRange + 1; colInd++) {
					if ((colInd >= 0) && (colInd < gridSize.col)) {
						Vector3 boxLoc = map.GetComponent<UiTiles>().lanes[rowInd + tile.row].GetComponent<UiRow>().rowTiles[colInd].transform.position;
			...
			for (int rowInd = 1; rowInd <= selectedAbility.GetComponent<Ability>().getAreaOfEffect(); rowInd++) {
				...
				for (int colInd = tile.col - rowRange; colInd < tile.col + rowRange + 1; colInd++) {
					if ((colInd >= 0) && (colInd < gridSize.col)) {
						Vector3 boxLoc = map.GetComponent<UiTiles>().lanes[tile.row - rowInd].GetComponent<UiRow>().rowTiles[colInd].transform.position;
		...
		if ((tile.row != -1) && (tile.col != -1) && (currentLevel.GetComponent<WaveList>().laneEnable[tile.row])) {
			...
			if (placementBoxes == null) {
				...
				for (int rowInd = 0; rowInd < gridSize.row; rowInd++) {
					for (int colInd = 0; colInd < gridSize.col; colInd++) {
						Vector3 boxLoc = map.GetComponent<UiTiles>().lanes[rowInd].GetComponent<UiRow>().rowTiles[colInd].transform.position;
			...
			if (placementBoxes != null) {
				foreach(GameObject ic in placementBoxes) {
					Destroy(ic);
				}
				placementBoxes = null;
			}
			...
			if ((tile.row != -1) && (tile.col != -1)) {
				if (tileContents[tile.row][tile.col] != null) {
					if (tileContents[tile.row][tile.col].GetComponent<GomUnit>().faction == GomObject.Faction.Player) {
						if (selectedAbility.GetComponent<Ability>().getUseCost() <= Player.spiritShards) {
							tileContents[tile.row][tile.col].SendMessage("SetInvincible", selectedAbility.GetComponent<Ability>().duration, SendMessageOptions.DontRequireReceiver);tileContents[tile.row][tile.col].SendMessage("SetInvincible", selectedAbility.GetComponent<Ability>().duration, SendMessageOptions.DontRequireReceiver);
							tileContents[tile.row][tile.col].SendMessage("SetInvincibleIcon", selectedUiAbility, SendMessageOptions.DontRequireReceiver);tileContents[tile.row][tile.col].SendMessage("SetInvincible", selectedAbility.GetComponent<Ability>().duration, SendMessageOptions.DontRequireReceiver);
							Player.spiritShards -= selectedAbility.GetComponent<Ability>().getUseCost();
